import joblib
import json
import numpy as np
import base64
import cv2
import os
from wavelet import w2d  # Ensure this is defined elsewhere


# Global Variables
__class_name_to_number = {}
__class_number_to_name = {}
__model = None


def class_number_to_name(class_num):
    return __class_number_to_name[class_num]


def load_saved_artifacts():
    print("🔄 loading saved artifacts...")

    global __class_name_to_number
    global __class_number_to_name
    global __model

    with open("./includes/class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v: k for k, v in __class_name_to_number.items()}

    if __model is None:
        with open('./includes/saved_model.pkl', 'rb') as f:
            __model = joblib.load(f)

    print("✅ loading saved artifacts...done")


def get_cv2_image_from_base64_string(b64str):
    try:
        encoded_data = b64str.split(',')[1] if ',' in b64str else b64str
        nparr = np.frombuffer(base64.b64decode(encoded_data), np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        return img
    except Exception as e:
        print(f"❌ Failed to decode base64 image: {e}")
        return None


def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    face_cascade = cv2.CascadeClassifier('./opencv/haarcascades/haarcascade_frontalface_default.xml')
    eye_cascade = cv2.CascadeClassifier('./opencv/haarcascades/haarcascade_eye.xml')

    img = None

    if image_path:
        if not os.path.exists(image_path):
            raise FileNotFoundError(f"Image path not found: {image_path}")
        img = cv2.imread(image_path)
    else:
        img = get_cv2_image_from_base64_string(image_base64_data)

    if img is None:
        raise ValueError("Image could not be loaded.")

    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.3, 5)

    cropped_faces = []
    for (x, y, w, h) in faces:
        roi_gray = gray[y:y + h, x:x + w]
        roi_color = img[y:y + h, x:x + w]
        eyes = eye_cascade.detectMultiScale(roi_gray)
        if len(eyes) >= 2:
            cropped_faces.append(roi_color)

    if len(cropped_faces) == 0:
        raise Exception("No face with two eyes detected.")

    return cropped_faces


def classify_image(img_base64_data, file_path=None):
    imgs = get_cropped_image_if_2_eyes(file_path, img_base64_data)
    result = []

    for img in imgs:
        # Resize raw image and wavelet-transformed image
        scaled_raw_img = cv2.resize(img, (32, 32))
        img_har = w2d(img, 'db1', 5)
        scaled_img_har = cv2.resize(img_har, (32, 32))

        # Stack and reshape
        combined_img = np.vstack((
            scaled_raw_img.reshape(32 * 32 * 3, 1),
            scaled_img_har.reshape(32 * 32, 1)
        ))
        len_image_array = 32 * 32 * 3 + 32 * 32
        final = combined_img.reshape(1, len_image_array).astype(float)

        # Predict
        prediction = __model.predict(final)[0]
        probabilities = np.around(__model.predict_proba(final) * 100, 2).tolist()[0]

        result.append({
            'class': class_number_to_name(prediction),
            'class_probability': probabilities,
            'class_dictionary': __class_name_to_number
        })

    return result


def get_b64_test_image():
    with open("b64.txt", "r") as f:
        return f.read()


def safe_classify(image_path=None, base64_data=None):
    try:
        return classify_image(base64_data, image_path)
    except Exception as e:
        return f"❌ Error processing {image_path or 'base64 image'}: {str(e)}"


if __name__ == '__main__':
    load_saved_artifacts()
    print(classify_image(get_b64_test_image(),None))
